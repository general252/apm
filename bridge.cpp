...
ApmHandle apm_create(APMConfig* param, int* err)
{
    ...
    cfg.transient_suppression.enabled = param->transient_suppression_enabled; // 瞬态噪声抑制。处理突然发生的尖锐噪声（如敲击桌子的声音）
...
int apm_process_reverse_stream(ApmHandle h, float* samples, int num_channels) {
    ...
    return inst->apm->ProcessReverseStream((const float* const*)samples, stream_cfg, stream_cfg, (float* const*)samples);
}
...
int apm_process_stream(ApmHandle h, float* samples, int num_channels) {
    ...
    return inst->apm->ProcessStream((const float* const*)samples, stream_cfg, stream_cfg, (float* const*)samples);
}
